...
class BaseLine(nn.Module):
    def __init__(self, hidden_dim, filter_size, dropout_rate, vocab_size, embedding_dim, num_layers, padding, pre_trained_embedding=None):
        ...
        self.bi_rnn = nn.LSTM(self.hidden_dim, int(self.hidden_dim / 2), num_layers=num_layers, batch_first=False, bidirectional=True)
        self.uni_rnn = nn.LSTM(self.hidden_dim, self.hidden_dim, num_layers=num_layers, batch_first=False)
    ...
    def forward(self, x):
        ...
        # x: (sentence_len-2, batch_size, hidden_dim)
        x = self.relu(x)
        x = self.dropout(x)
        x_res = x
        x, _ = self.bi_rnn(x)
